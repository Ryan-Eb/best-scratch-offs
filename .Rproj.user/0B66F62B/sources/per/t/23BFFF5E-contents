...
daily_csv <- list.files("/Users/jennaryan/Documents/R Projects/nc lottery/daily_csv_nc",
                        full.names = TRUE) %>% 
  lapply(read_csv)
...
clean_df <- df %>% 
  ...
  mutate(start_overall_odds = as.numeric(str_sub(overall_odds, start = 6L, end =-1L))) %>% 
  ...
  mutate(start_date = mdy(launch_date)) %>% 
